      ...
      {viewingPhotoUrl && (
        <div className="fixed inset-0 z-[60] bg-black/80 flex items-center justify-center p-4" onClick={() => setViewingPhotoUrl(null)}>
          <div className="relative max-w-4xl max-h-[90vh] w-full" onClick={(e) => e.stopPropagation()}>
            <button onClick={() => setViewingPhotoUrl(null)} className="absolute -top-4 -right-4 z-10 w-10 h-10 rounded-full bg-white text-gray-800 flex items-center justify-center shadow-xl hover:bg-gray-100 transition">
              <X size={20} />
            </button>
      ...
      <aside className="w-64 bg-white border-r border-gray-200 flex flex-col flex-shrink-0">
        <div className="p-5 border-b border-gray-100">
          <div className="flex items-center gap-2 mb-1">
            <Users size={20} className="text-[#2F6F4F]" />
            <h2 className="text-lg font-bold text-gray-800">대상자 목록</h2>
          </div>
          <p className="text-xs text-gray-400">인터뷰 대상자를 관리합니다</p>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-3 space-y-1">
          ...
          >
            <List size={16} />
            전체 보기
            <span className="ml-auto text-xs bg-gray-200 text-gray-600 px-2 py-0.5 rounded-full">{questions.length}</span>
          </button>
          ...
          {persons.map(person => {
            ...
            return (
              <div key={person.id} className="group relative">
                ...
                >
                  <span className="w-7 h-7 rounded-full bg-gradient-to-br from-[#2F6F4F] to-[#4a9d6e] text-white flex items-center justify-center text-xs font-bold flex-shrink-0">
                    {person.name.charAt(0)}
                  </span>
                  <span className="truncate">{person.name}</span>
                  <span className="ml-auto text-xs bg-gray-200 text-gray-600 px-2 py-0.5 rounded-full flex-shrink-0">{count}</span>
                </button>
                <button
          ...
          {showPersonForm ? (
            <div className="space-y-2">
              <input
                ...
                className="w-full p-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:border-[#2F6F4F]"
                value={newPersonName}
              ...
              <div className="flex gap-2">
                <button onClick={handleCreatePerson} className="flex-1 py-2 bg-[#2F6F4F] text-white text-sm font-bold rounded-lg hover:bg-[#23533b] transition">추가</button>
                <button onClick={() => { setShowPersonForm(false); setNewPersonName(''); setNewPersonDesc(''); }} className="flex-1 py-2 bg-gray-100 text-gray-600 text-sm font-bold rounded-lg hover:bg-gray-200 transition">취소</button>
              </div>
            </div>
          ) : (
            <button
              onClick={() => setShowPersonForm(true)}
              className="w-full flex items-center justify-center gap-2 px-4 py-2.5 bg-gray-100 text-gray-600 rounded-xl text-sm font-bold hover:bg-gray-200 transition"
            >
              <UserPlus size={16} />
              새 대상자 추가
            </button>
          ...
          <div className="bg-white p-6 md:p-8 rounded-2xl shadow-sm border border-gray-100">
            ...
            {showForm && (
              <div className="mb-8 bg-[#f9fdfa] p-6 rounded-2xl border border-[#2F6F4F]/20">
                <h3 className="text-lg font-bold text-gray-800 mb-5">새 질문 만들기</h3>
                <div className="flex flex-col gap-5">
                  ...
                  {!selectedPersonId ? (
                    <div>
                      ...
                      />
                      <p className="text-xs text-gray-400 mt-1">💡 왼쪽 사이드바에서 대상자를 먼저 추가하면 자동으로 연결됩니다.</p>
                    </div>
                  ) : (
                    <div className="flex items-center gap-2 px-3 py-2 bg-[#eef5f0] rounded-lg">
                      <span className="w-6 h-6 rounded-full bg-[#2F6F4F] text-white flex items-center justify-center text-xs font-bold">
                        {selectedPerson?.name?.charAt(0)}
                      </span>
                      <span className="text-sm font-bold text-[#2F6F4F]">{selectedPerson?.name} 대상자에게 질문</span>
                    </div>
                  )}
                  ...
                  {/* 메인 질문 내용 */}
                  <div>
                    <label className="block text-sm text-gray-600 mb-2 font-bold">메인 질문</label>
                    <textarea
                      className="w-full p-3 border border-gray-300 rounded-xl bg-white focus:outline-none focus:border-[#2F6F4F] transition shadow-sm text-gray-800 font-medium resize-none"
                      rows={2}
                  ...
                  {/* 질문 목적 */}
                  <div>
                    <label className="block text-sm text-gray-600 mb-2 font-bold">이 질문이 노리는 것 <span className="font-normal text-gray-400">(관리자 메모, 선택)</span></label>
                    <input
                      type="text"
                      className="w-full p-3 border border-gray-300 rounded-xl bg-white focus:outline-none focus:border-[#2F6F4F] transition shadow-sm text-gray-800 font-medium"
                  ...
                  <div>
                    <div className="flex items-center justify-between mb-3">
                      <label className="text-sm text-gray-600 font-bold">꼬리질문 설계 <span className="font-normal text-gray-400">(선택, AI 가이드용)</span></label>
                      <button
                        onClick={addFollowUpField}
                        className="flex items-center gap-1 text-xs font-bold text-[#2F6F4F] hover:text-[#23533b] transition px-3 py-1.5 rounded-lg bg-[#eef5f0] hover:bg-[#d8e3da]"
                      >
                    ...
                    <div className="space-y-3 mt-2">
                      {newFollowUps.map((fu, i) => (
                        <div key={i} className="flex gap-2 items-start bg-white p-3 rounded-xl border border-gray-200 shadow-sm">
                          <div className="flex items-center justify-center w-7 h-7 rounded-full bg-[#eef5f0] text-[#2F6F4F] text-xs font-bold flex-shrink-0 mt-1">
                            {i + 1}
                          </div>
                          <div className="flex-1 space-y-2">
                            <input
                              type="text"
                              className="w-full p-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:border-[#2F6F4F]"
                            ...
                            <input
                              type="text"
                              className="w-full p-2 border border-gray-100 rounded-lg text-xs text-gray-500 focus:outline-none focus:border-[#2F6F4F]"
                  ...
                  {/* 사진 요청 문구 */}
                  <div>
                    <label className="block text-sm text-gray-600 mb-2 font-bold">📸 사진 요청 문구 <span className="font-normal text-gray-400">(대화 완료 후 표시, 선택)</span></label>
                    <input
                      type="text"
                      className="w-full p-3 border border-gray-300 rounded-xl bg-white focus:outline-none focus:border-[#2F6F4F] transition shadow-sm text-gray-800 font-medium"
                  ...
                  <div key={q.id} className="rounded-2xl border border-gray-200 shadow-sm overflow-hidden bg-white">
                    ...
                    <div className="p-5 flex flex-col sm:flex-row gap-4">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        {q.purpose && (
                          <p className="text-[11px] text-gray-400 mt-1.5 italic">🎯 {q.purpose}</p>
                        )}
                        {q.photo_request && (
                          <p className="text-[11px] text-blue-400 mt-1">📸 {q.photo_request}</p>
                        )}
                    ...
                    {viewingFollowUpsId === q.id && (
                      <div className="border-t border-gray-100 bg-amber-50/40 p-5">
                        <div className="ml-2 pl-4 border-l-4 border-amber-400/40">
                          ...
                          {followUpsLoading ? (
                            ...
                            <div className="bg-white p-4 rounded-xl border border-gray-200 shadow-sm inline-block">
                              <p className="text-sm text-gray-400 font-medium">등록된 꼬리질문이 없습니다. AI가 자유롭게 생성합니다.</p>
                            </div>
                          ) : (
                            <div className="flex flex-col gap-3">
                              {followUpTemplates.map((fu, i) => (
                                <div key={fu.id} className="bg-white p-4 rounded-xl border border-gray-200 shadow-sm relative">
                                  <div className="absolute top-4 left-4 w-6 h-6 rounded-full bg-amber-100 text-amber-700 flex items-center justify-center text-xs font-bold">
                                    {i + 1}
                                  </div>
                                  <div className="pl-9">
                                    <p className="text-gray-800 font-medium text-[14px]">"{fu.follow_up_text}"</p>
                                    {fu.guide_note && (
                                      <p className="text-[11px] text-gray-400 mt-1.5 italic">💡 {fu.guide_note}</p>
                                    )}
                                  </div>
                                </div>
                              ))}
